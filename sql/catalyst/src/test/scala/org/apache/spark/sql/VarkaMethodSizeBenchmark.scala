 ...
 *
 * The arm is named `single epilogue` explicitly, so that when the plan's per-group epilogue
...
object VarkaMethodSizeBenchmark extends BenchmarkBase {
  ...
  private def emit(fused: CompiledVarkaProjection, loader: VarkaGeneratedClassLoader,
    ...
    val name = s"org.apache.spark.sql.varka.execution.VarkaFusedSizeBench$n"
    ...
    loader.defineGeneratedClass(name, VarkaLoopEmitter.emit(name, javaRoots,
      fused.inputOrdinals.size, fused.numLiterals, null, null, VarkaEmitOptions.DEFAULTS))
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    ...
    try {
      ...
      runBenchmark("the epilogue past HugeMethodLimit: a make_date ladder (task 87)") {
        for (n <- rungs) {
          val fused = shape(n)
          val kernel = emit(fused, loader, n)
          ...
          def drive(chunk: Int, mixed: Boolean): Int = {
            var status = 0
            var done = 0
            while (done < numRows) {
          ...
          for ((chunk, _) <- chunks; (mixed, _) <- nullArms) {
            val status = drive(chunk, mixed)
            require(status == 0,
              s"$n outputs at chunk $chunk (mixed=$mixed) declined with status $status")
          }
          ...
          val benchmark = new Benchmark(s"$n make_date outputs over $numRows rows in chunks",
            ...
            output = output)
          for ((chunk, note) <- chunks; (mixed, arm) <- nullArms) {
            benchmark.addCase(s"single epilogue, chunk $chunk ($note), $arm") { _ =>
              drive(chunk, mixed)
            }
          }
          benchmark.run()
