...
| 192 | Can vanilla Spark tune its way off the cliff? **Laptop runs done** (`PLAN_TASK_192.md` 9.1, 24 September 2026): no longer *cannot* - `hugeMethodLimit=8000` and `wholeStage=false` turn the step into about 1.1 times the pre-cliff cost per entry, and `-XX:-DontCompileHugeMethods` removes it up to 80 entries but not at a hundred (row 201); none comes within an order of magnitude of Varka's line. The band and the runner's runs remain | task 171's ladder | small, measured |
...
| 196 | *Research, optional.* Whether the cliff holds on every JDK Spark supports. **JDK 17 and 25 done** (`PLAN_TASK_192.md` 9.2): on stock Spark 4.2.0 the consume method is never compiled past 8000 bytes on either; the fork is built for Java 25 only, so the stock distribution is the one asked. JDK 21 remains. Measured on JDK 25 only; `HugeMethodLimit` has long been 8000, so the expectation is yes. Run `VarkaSizeLadderJitSuite` once on JDK 17 and 21 in CI, so "on every supported JDK" is a measured statement | the task 171 discussion | small |
