...
| G3 | Schema too wide at the source (coupling) | same `maxFields` test on the scan's schema; the cache uses the FULL cached-relation schema | Scan stops producing columnar output: FileSourceScan, Parquet/ORC v2, InMemoryTableScan | No | **NOT immune (proposal)**: Varka needs `child.supportsColumnar`, so a cached relation with >100 fields silently gets no Varka and no reason is recorded. *Corrected 25 September 2026 (`PLAN_TASK_185.md` 8.5): under Varka with the Arrow cache the scan counts the columns a query reads, so it is solved there; a reason is logged where a user can act on one* |
